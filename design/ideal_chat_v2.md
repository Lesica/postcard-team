...
1. **Storage**: events.jsonl per-day или single? Ротация по размеру?
2. **Конкуррентность**: если два процесса пишут в events.jsonl — нужен ли lock или atomic-append достаточно?
3. **Тема-в-head**: один чат-вкладка на каждую (head × topic)? Это 3 голов × 3 топика = 9 вкладок? Или меньше?
4. **Boot-файл формат**: markdown vs structured (yaml/json)? Markdown для людей читабельнее.
5. **Capacity threshold**: 80% — а как мерить? Tokens или blocks? Char count?
6. **Лара (CLI) в TG**: дать ей выделенный топик или личный канал с ботом?
